...
@interface FxParameter (FxParameterBaseTests)
...
@interface NSCoder (FxParameterBaseTests)
...
@interface FxParameterBaseTests : XCTestCase
...
@implementation FxParameterBaseTests
...
{
	return FxParamClassTestConfig(kBaseTestParameter, kFxParameterType_Float, @"Amount", extra);
}

- (FxGripFloatParameter *)makeParameterWithID:(FxParameterId)parameterID extra:(NSDictionary *)extra
{
	NSMutableDictionary *config = FxParamClassTestConfig(parameterID, kFxParameterType_Float, @"Amount", extra);
...
{
	NSDictionary *config = FxParamClassTestConfig(kBaseTestParameter, kFxParameterType_Toggle, @"Amount", nil);
...
- (void)testTheChangeBracketAcceptsEveryTime
{
	...
	XCTAssertTrue([parameter startChangedTime:FxParamClassTestTime(0, 1) error:&error]);
	XCTAssertTrue([parameter endChangedTime:FxParamClassTestTime(0, 1) error:&error]);
	XCTAssertNil(error);
}
...
{
	FxGripFloatParameter *parameter = [self makeParameterWithExtra:nil];
	NSError *failure = [NSError errorWithDomain:@"FxParameterBaseTest" code:2 userInfo:nil];
...
{
	NSDictionary *config = FxParamClassTestConfig(kBaseTestParameter, kFxParameterType_PushButton, @"Reset", nil);
...
{
	NSDictionary *parentConfig = FxParamClassTestConfig(kBaseTestParentParameter, kFxParameterType_Float, @"Group",
...
{
	NSDictionary *parentConfig = FxParamClassTestConfig(kBaseTestParentParameter, kFxParameterType_Float, @"Group", nil);
	FxGripFloatParameter *parent = [FxGripFloatParameter.alloc initWithDictionary:parentConfig
																		   effect:(id)self.effect];
	[self.retainedParameters addObject:parent];
	self.effect.parameters[@(kBaseTestParentParameter)] = parent;
...
{
	FxGripFloatParameter *parameter = [self makeParameterWithExtra:nil];
	CMTime timeline = FxParamClassTestTime(0, 1);

	[parameter timelineTime:&timeline fromImageTime:FxParamClassTestTime(6, 30)];
...
{
	FxGripFloatParameter *parameter = [self makeParameterWithExtra:nil];
	CMTime image = FxParamClassTestTime(0, 1);

	[parameter imageTime:&image fromTimelineTime:FxParamClassTestTime(12, 30)];
...
- (void)testAPluginStateCoderReadsTheValueAtTheCodersRenderTime
{
	FxGripFloatParameter *parameter = [self makeParameterWithExtra:nil];
	NSKeyedArchiver *archiver = [NSKeyedArchiver.alloc initRequiringSecureCoding:NO];
	archiver.renderTime = FxParamClassTestTime(13, 30);
...
- (void)testACachingParameterAnswersTheFlagsQueryFromItsStagedFlags
{
	...
	NSMutableDictionary *userInfo = [self postFlagsNotification:FxNotifyAPI_ParameterGetFlagsPreName
													  parameter:parameter
...
- (void)testAFlagsWriteThatTurnsCachingOnIsAbsorbedIntoTheStagedFlags
{
	...
	NSMutableDictionary *setUserInfo = [self postFlagsNotification:FxNotifyAPI_ParameterSetFlagsPreName
														 parameter:parameter
	...
	NSMutableDictionary *getUserInfo = [self postFlagsNotification:FxNotifyAPI_ParameterGetFlagsPreName
														 parameter:parameter
															 flags:kFxParameterFlag_DEFAULT];
...
- (void)testAFlagsWriteWithoutCachingLosesOnlyTheDirtyMarkerAndReachesTheHost
{
	...
	NSMutableDictionary *userInfo = [self postFlagsNotification:FxNotifyAPI_ParameterSetFlagsPreName
													  parameter:parameter
...
- (void)testACompletedSavingWriteBecomesTheParametersCurrentFlags
{
	...
	[self postFlagsNotification:FxNotifyAPI_ParameterSetFlagsName
					  parameter:parameter
...
- (void)testACachingParametersWriteThroughTheWrapperStagesWithoutMovingTheCurrentFlags
{
	...
	NSMutableDictionary *query = [self postFlagsNotification:FxNotifyAPI_ParameterGetFlagsPreName
												  parameter:parameter
													  flags:kFxParameterFlag_DEFAULT];
