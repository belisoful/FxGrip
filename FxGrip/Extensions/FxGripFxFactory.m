...
- (instancetype _Nullable)init
{
	...
	if (self) {
		...
		_fxFactoryIsInstalled = [self factoryInstalled];
	}
	return self;
}
...
#pragma mark -
#pragma mark FxFactory SDK bridge (overridable test seams)

// Every FxFactory SDK entry point is reached through one of these methods. A subclass
// overrides them with a mock to drive the licensing, watermark, and update logic end to end
// without a live FxFactory installation. The FxFactory symbols are weak-linked, so each
// guards its own null symbol.

- (BOOL)factoryInstalled
{
	return FxFactoryIsInstalled();
}

- (FxFactoryVersion)factoryVersion
{
...
- (FxFactoryLicensingStatus)factoryLicensingStatusForProduct:(NSString *)productUUID
{
...
- (nullable id)factoryRegisterLicensingHandlerForProduct:(NSString *)productUUID
												 handler:(void (^)(FxFactoryLicensingStatus, id _Nullable))handler
{
	if (FxFactoryRegisterLicensingStatusChangeHandler == NULL) {
		return nil;
	}
	return FxFactoryRegisterLicensingStatusChangeHandler(productUUID, self, handler);
}
...
- (void)factoryShowProductUpdatesForProduct:(NSString *)productUUID
									version:(NSString *)version
									  force:(BOOL)force
									handler:(void (^)(NSDictionary * _Nonnull))handler
{
...
- (BOOL)factoryPerformLicensingAction:(FxFactoryLicensingAction)action forProduct:(NSString *)productUUID
{
	if (FxFactoryPerformLicensingAction == NULL) {
		return NO;
	}
	return FxFactoryPerformLicensingAction(action, productUUID);
}
...
- (void)showProductUpdates:(BOOL)forceCheck handler:(void (^)(NSDictionary * _Nonnull response))handler
{
	...
	^(NSDictionary * productInfo) {
		if (productInfo != nil) {
			[self.fxEffect onFxFactoryShowProductUpdates:productInfo];
	...
	[self factoryShowProductUpdatesForProduct:self.fxFactoryPluginUUID version:self.fxFactoryPluginVersion force:forceCheck handler:_handler];
}
...
- (void)extAddParameters:(nonnull NSNotification*)notification
{
	...
	// Find the fxfactory parameter. The type is matched as a string: the guarded
	// parameterType accessor maps only the standard types, so the custom FxFactory type
	// would read as None and the entry would be missed.
	for(NSMutableDictionary *pcheck in parameters) {
		if ([pcheck[kFxParameterProperty_Type] isEqualToString:kFxParameterType_FxFactory]) {
			parameter = pcheck;
			break;
		}
	}
	...
	if (!_fxFactoryPluginUUID) {
		...
		if (!_fxFactoryPluginUUID) {
			_fxFactoryPluginUUID = self.fxEffect.pluginProperties[kPropertiesFxFactoryPluginUUID];
		}
		if (_fxFactoryPluginUUID && [_fxFactoryPluginUUID isKindOfClass:NSNumber.class]) {
			_fxFactoryPluginUUID = self.fxEffect.pluginUUID;
		}
	}
	...
	if (_fxFactoryPluginUUID) {
		...
		if (!_fxFactoryPluginVersion) {
			...
			if (!_fxFactoryPluginVersion) {
				_fxFactoryPluginVersion = self.fxEffect.pluginProperties[kPropertiesFxFactoryPluginVersion];
			}
			if (_fxFactoryPluginVersion && [_fxFactoryPluginVersion isKindOfClass:NSNumber.class]) {
				_fxFactoryPluginVersion = self.fxEffect.pluginStringVersion;
			}
		}
	}
...
- (void)extAddedToDocument:(nonnull NSNotification*)notification
{
	...
	if (debugLicensed && [debugLicensed isKindOfClass:NSNumber.class]) {
		...
		// The debug override drives the whole licensing decision: seed the cached status so
		...
		if (currentLicenseStatus != priorLicenseStatus) {
			...
			[self.fxEffect setFxFactoryLicenseState:currentLicenseStatus];
		}
#if DEBUG
	}
#endif
...
{
	return [self factoryUpdateCheckingEnabledForProduct:self.fxFactoryPluginUUID];
}
...
- (void)setPluginIsUpdateChecking: (BOOL)enabled
{
	...
	[self factorySetUpdateCheckingEnabled:enabled forProduct:self.fxFactoryPluginUUID];
}
...
- (BOOL)showFxFactoryProduct
{
	...
	return [self factoryPerformLicensingAction:kFxFactoryLicensingActionShow forProduct:self.fxFactoryPluginUUID];
}
- (BOOL)showFxFactoryBuyPlugin
{
	[self.effect.notifier postNotificationName:kFxFactoryBroadcastShowBuy object:self];
	return [self factoryPerformLicensingAction:kFxFactoryLicensingActionBuy forProduct:self.fxFactoryPluginUUID];
}
...
- (id<FxGripFxFactorySettings> )fxFactorySettingsObject
{
	if (!_fxFactorySettingsObject) {
		if ([self.effect conformsToProtocol:@protocol(FxGripFxFactorySettings)]) {
			return (id<FxGripFxFactorySettings>) self.effect;
		}
		return NULL;
	}
...
{
	return (FxGripFxFactory*)[self extensionForClass:FxGripFxFactory.class];
}
...
{
	return [FxGripFxFactory.alloc init];
}
