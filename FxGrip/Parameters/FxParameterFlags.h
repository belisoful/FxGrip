...
#ifndef FxParameterFlags_h
#define FxParameterFlags_h
...
typedef union FxGripParameterFlags {
	struct {
		...
		unsigned int _reserved7:1;		//	bit 7			  Is this incorrectly labeled 17?
		...
		unsigned int useFullViewWidth:1;	//	bit 11
			...
			unsigned int _reserved22:1;		//	bit 22
		   	unsigned int _reserved23:1;		//	bit 23
		
		unsigned int saving:1;				//	bit 24
		unsigned int cacheDirty:1;			//	bit 25
		unsigned int cache:1;				//	bit 26
		
		unsigned int hiddenProxy:1;			//	bit 27
		unsigned int inDebugMode:1;			//	bit 28
		unsigned int noDebug:1;				//	bit 29
		
		unsigned int noRenderState:1;		//	bit 20
...
/* #if __LP64__
	...
	#define	kFxParameterFlag_NOSTATE		(((FxParameterFlags64) 1) << 30)

// NO_DEBUG is to not show the parameter in debugmode
	#define	kFxParameterFlag_NO_DEBUG		(((FxParameterFlags64) 1) << 29)
	#define	kFxParameterFlag_IN_DEBUG_MODE	(((FxParameterFlags64) 1) << 28)
    #define	kFxParameterFlag_HIDDEN_PROXY	(((FxParameterFlags64) 1) << 27)
...
//Real time flags - CHANGED and VALUEUPDATED are not saved, masked off by APP_MASK
	...
	#define	kFxParameterFlag_CACHE			(((FxParameterFlags64) 1) << 26)
	#define	kFxParameterFlag_CACHEDIRTY	(((FxParameterFlags64) 1) << 25)
	//This is when setting the flags inside the
	#define	kFxParameterFlag_SAVING			(((FxParameterFlags64) 1) << 24)

// All of the above bits are in this mask.
#define	kFxParameterFlag_APP_MASK		(0xFF000000)
#define	kFxParameterFlag_FX_MASK		( 0x00007FF )
...
#define FxParameterFlagsFxMask(m) ((FxParameterFlags)(( \
