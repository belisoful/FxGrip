...
{
	hasMeta(nil);
	return FxGripHostMeta(self.effect).tags;
}

- (SInt32)tagCount
{
	hasMeta(0);
	return [FxGripHostMeta(self.effect) tagCount];
}

- (SInt32)tagCount:(FxParameterId)parameterID
{
	hasMeta(-1);
	return [FxGripHostMeta(self.effect) tagCount:parameterID];
}

- (NSArray<NSString*>* _Nullable)parameterTags:(FxParameterId)parameterID
{
	hasMeta(nil);
	return [FxGripHostMeta(self.effect) parameterTags:parameterID];
}
...
- (BOOL)parameter:(FxParameterId)parameterID hasTag:(NSString* _Nullable)tag error:(NSError* _Nullable * _Nullable)error
{
	...
	return [FxGripHostMeta(self.effect) parameter:parameterID hasTag:tag error:error];
}
...
- (NSError* _Nullable)setTags:(NSArray<NSString*>*_Nonnull)tags toParameter:(FxParameterId)parameterID
{
	...
	return [FxGripHostMeta(self.effect) setTags:tags toParameter:parameterID];
}

- (NSError* _Nullable)addTag:(NSString*_Nullable)tag toParameter:(FxParameterId)parameterID
{
	hasMeta(noMetaError(parameterID));
	return [FxGripHostMeta(self.effect) addTag:tag toParameter:parameterID];
}

- (NSError* _Nullable)removeTag:(NSString*_Nullable)tag fromParameter:(FxParameterId)parameterID
{
	hasMeta(noMetaError(parameterID));
	return [FxGripHostMeta(self.effect) removeTag:tag fromParameter:parameterID];
}

- (NSError* _Nullable)removeAllTags:(FxParameterId)parameterID
{
	hasMeta(noMetaError(parameterID));
	return [FxGripHostMeta(self.effect) removeAllTags:parameterID];
}

- (NSArray* _Nullable)parametersWithTag:(NSString*_Nullable)tag
{
	hasMeta(nil);
	return [FxGripHostMeta(self.effect) parametersWithTag:tag];
}
								  ...
								  record:(NSDictionary *_Nullable *_Nullable)record
{
	...
	NSDictionary *source = FxGripHostHasMeta(effect) ? [FxGripHostMeta(effect) parameterData:parameterID] : nil;
	if (!source) {
							  ...
							  tag:(NSString *_Nullable)tag
{
	...
	if ((options & FxGripPresetFlags) && [flagSpecs isKindOfClass:NSDictionary.class]) {
		...
		for (NSNumber *pid in FxGripSectionParameterIDs(flagSpecs)) {
			...
			FxParameterFlags current = [FxGripHostParameterData(effect) storedFlags:pid.unsignedIntValue];
	...
	if ((options & FxGripPresetTags) && [tagSpecs isKindOfClass:NSDictionary.class]) {
		for (NSNumber *pid in FxGripSectionParameterIDs(tagSpecs)) {
			...
			if (flagNoTags([FxGripHostParameterData(effect) storedFlags:pid.unsignedIntValue])) {
				continue;
			}
	...
	if ((options & FxGripPresetMeta) && [metaSpecs isKindOfClass:NSDictionary.class]) {
		for (NSNumber *pid in FxGripSectionParameterIDs(metaSpecs)) {
			...
			if (flagNoMeta([FxGripHostParameterData(effect) storedFlags:pid.unsignedIntValue])) {
				continue;
			}
			...
			for (NSString *key in entries) {
				if (![FxGripHostMeta(effect) setMeta:entries[key] forKey:key toParameter:pid.unsignedIntValue]) {
	...
	if ((options & FxGripPresetNames) && [names isKindOfClass:NSDictionary.class]) {
		id<FxDynamicParameterAPI_v4> dynamicAPI = effect.apiManager.dynamicParamAPIv4;
										   ...
										   index:(int)index
{
	...
	id<FxGripEffectHost> effect = self.effect;
	NSArray *entries = [FxGripHostParameterData(effect) storedMenus:parameterID];
