...
- (id)apiForProtocol:(Protocol *)apiProtocol bypass:(BOOL)bypassFxGripLayer
{
	...
	// No FxDynamicParameterAPI_v4, yet.
	if (!api && [FxGripDynamicParameterAPI_v4 conformsToProtocol:apiProtocol] ) {
		id<FxDynamicParameterAPI_v3> dyParam_v3 = self.dynamicParamAPIv3_Raw;
		if (dyParam_v3 == nil) {
			NSLog(@"FxGripAPIManager(%llu)::apiProtocol Unable to load FxGripDynamicParameterAPI_v3 for FxGripDynamicParameterAPI_v4", _sessionID);
			return nil;
		}
			
		id dyParam_v4 = NARC_AUTORELEASE([FxGripDynamicParameterAPI_v4.alloc initWithAPI:dyParam_v3 effect:self.effect]);
		if (dyParam_v4 == nil)
		{
			NSLog(@"FxGripAPIManager(%llu)::apiProtocol Unable to load FxGripDynamicParameterAPI_v4", _sessionID);
		}
		return dyParam_v4;
	}
	...
	if (api && [FxGripParameterRetrievalAPI_v6 conformsToProtocol:apiProtocol]) {
		...
		if (!paramGetAPI_v6) {
			...
			paramGetAPI_v6 = NARC_AUTORELEASE([FxGripParameterRetrievalAPI_v6.alloc initWithAPI:api dynamicParamAPIv4:dyParam_v4 effect:self.effect]);
	...
	if (api && [FxGripParameterSettingAPI_v5 conformsToProtocol:apiProtocol]) {
		...
		id<FxDynamicParameterAPI_v4> dyParam_v4 = NARC_AUTORELEASE([FxGripDynamicParameterAPI_v4.alloc initWithAPI:self.dynamicParamAPIv3_Raw effect:self.effect]);
		if (dyParam_v4 == nil)
		{
			NSLog(@"FxGripAPIManager(%llu)::apiProtocol Unable to load FxDynamicParameterAPI_v4", _sessionID);
		}
		
		id paramSetAPI_v5 = NARC_AUTORELEASE([FxGripParameterSettingAPI_v5.alloc initWithAPI:api paramGetAPIv6:paramGetAPIv6 dynamicParamAPIv4:dyParam_v4 effect:self.effect]);
	...
	if (api && [FxGripParameterSettingAPI_v6 conformsToProtocol:apiProtocol]) {
		...
		id paramSetAPI_v6 = NARC_AUTORELEASE([FxGripParameterSettingAPI_v6.alloc initWithAPI:api paramGetAPIv6:paramGetAPIv6 dynamicParamAPIv4:dyParam_v4 effect:self.effect]);
...
- (id<FxDynamicParameterAPI_v4> _Nullable)dynamicParamAPIv4_Raw
{
	return [_apiAccessing apiForProtocol:@protocol(FxDynamicParameterAPI_v4)];
}

- (id<FxDynamicParameterAPI_v3> _Nullable)dynamicParamAPIv4
{
	id<FxDynamicParameterAPI_v4> paramAPI = [self apiForProtocol:@protocol(FxDynamicParameterAPI_v4)];
	if (paramAPI == nil)
	{
		NSLog(@"FxGripAPIManager(%llu)::dynamicParamAPIv4 Unable to load FxDynamicParameterAPI_v4", _sessionID);
	}
	return paramAPI;
}
