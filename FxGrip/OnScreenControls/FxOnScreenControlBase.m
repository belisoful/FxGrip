...
@interface FxOnScreenControlBase ()
{
...
@implementation FxOnScreenControlBase
...
- (nullable id<MTLLibrary>)fxOSCLibraryForDevice:(nullable id<MTLDevice>)device
{
	...
	NSError *error = nil;
	library = [device newDefaultLibraryWithBundle:[NSBundle bundleForClass:FxOnScreenControlBase.class]
											error:&error];
	if (library == nil) {
								   ...
								   device:(nonnull id<MTLDevice>)device
{
	...
	CTFontRef font = CTFontCreateWithName(CFSTR("Helvetica"), clampedSize, NULL);
	...
	CGRect bounds = [attributed boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, CGFLOAT_MAX)
	...
	[attributed drawAtPoint:NSMakePoint(padding, padding)];
					  ...
					  atTime:(CMTime)time
{
	...
	*forceUpdate = NO;
	...
	if (activePart != 0) {
		for (FxGripOSCPart *part in _parts) {
			if (part.partID == activePart) {
				*forceUpdate = [part mouseDownAtObjectPoint:objectPoint
												canvasPoint:canvasPoint
												  modifiers:modifiers
													 atTime:time];
				break;
						 ...
						 atTime:(CMTime)time
{
	...
	// Both constrain and fine-drag are applied here in the base, so every part inherits them.
